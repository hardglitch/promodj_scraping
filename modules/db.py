...
async def write_file_history(link: str, date: int) -> None:
    if link is None or not isinstance(link, str):
        return debug.log(Messages.Errors.NoLinkToWrite)
    if date is None or not isinstance(date, int):
        return debug.log(Messages.Errors.NoDate)
...
async def filter_by_history(found_links: Set[str]) -> Set[str]:
    if not isinstance(found_links, Set) or not all(map(lambda x: True if type(x)==str else False, found_links)):
