...
@allure.epic("Bruteforce the Manager")
class BruteforceManager:
    ...
    BASE_TYPES: Tuple[Any] = (1, -1, 0, 0.1, range(10000), "s")
    ...
    def hard_test(self, obj: Callable, params_range: int = 5, cycles: int = 100, mode: int = FULL_CHAOS) -> bool:
        ...
        def recursive_func(*args, n: int = 0):
            for tp in self.TEST_PARAMETERS:
                ...
                try:
                    if mode == self.FULL_CHAOS: _ = obj(*args, tp)
                    else: _ = obj(self.MockSettings(*args, tp))
    ...
    def test_start_settings_full_chaos(self, setup):
        assert self.hard_test(obj=self.MockMainWindow, mode=self.FULL_CHAOS)

    def test_start_settings_half_legal_arguments(self, setup):
        assert self.hard_test(obj=self.MockMainWindow, mode=self.HALF_LEGAL_ARGUMENTS)

    def test_start_settings_legal_arguments(self, setup):
        assert self.hard_test(obj=self.MockMainWindow, mode=self.LEGAL_ARGUMENTS)
