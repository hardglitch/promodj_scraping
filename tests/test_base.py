...
def test_default_values():
    b = Base()
    assert b.download_dir == Data.DefaultValues.download_dir
    ...
    assert b.progress == pyqtSignal(int)
    assert b.succeeded == pyqtSignal(int)
    assert b.total_size == 0
    assert b.total_downloaded == 0

def test_default_types():
    b = Base()
    assert isinstance(b.download_dir, str)
